...
import { Plus, Check, X, Users, Database, BarChart3, Upload, Link, Trash2, Loader2 } from "lucide-react"
...
export default function AdminPage() {
  ...
  const handleAddSource = async () => {
    if (!newSource.title || (!newSource.url && !selectedFile)) return

    setSubmittingSource(true)
    try {
      const formData = new FormData()
      formData.append('title', newSource.title)
      formData.append('category', newSource.category)
      formData.append('submittedBy', user?.id || 'admin')
      formData.append('submittedByEmail', user?.email || 'admin@sogestmatic.com')

      if (selectedFile) {
        formData.append('type', 'pdf')
        formData.append('file', selectedFile)
        if (newSource.url) formData.append('url', newSource.url)
      } else {
            ...
            <TabsContent value="sources" className="space-y-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Ajoutez des PDFs ou URLs pour enrichir les réponses de l'IA
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        placeholder="URL de la source (ex: https://legifrance.gouv.fr/...)"
                        value={newSource.url}
                  ...
                  <Button onClick={handleAddSource} disabled={submittingSource} className="bg-green-accent hover:bg-green-accent-dark">
                    {submittingSource ? (
                      <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                    ) : (
                      <Plus className="h-4 w-4 mr-2" />
                    )}
                    {submittingSource ? 'Ajout en cours...' : 'Ajouter la source'}
                  </Button>
                </CardContent>
              </Card>
