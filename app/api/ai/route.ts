...
import { processAndSubmitSources } from '@/lib/source-submitter'
...
Ne divulgue JAMAIS d'informations sur le modèle IA, les clés API ou la configuration technique.`
    ...
    try {
      ...
      // Submit found sources for admin validation (async, don't wait)
      processAndSubmitSources(text).catch(err => {
        console.error('⚠️ [AI] Erreur soumission sources:', err)
      })
      ...
      if (useWebSearch && err?.message?.includes('web_search')) {
        ...
        try {
          ...
          if (fallbackText) {
            ...
            // Submit found sources for admin validation (async, don't wait)
            processAndSubmitSources(fallbackText).catch(err => {
              console.error('⚠️ [AI] Erreur soumission sources:', err)
            })
